...
Có thể thử đưa /bin/sh vào vùng .BSS. Để làm được điều đó cần có địa chỉ của 1 vùng nhớ trống để ghi đè /bin/sh vào trong khi PIE đang bật.
...
Bây giờ cần nhập .bin/sh vào địa chỉ .BSS. Nhưng như thế cần setup hàm read để ghi đúng vào buffer do ta chỉ định. Cần leak địa chỉ hàm read.
Khi đã có địa chỉ read thì cần các gadget để truyền tham số.
